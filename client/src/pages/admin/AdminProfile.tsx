...
import { fetchMe, type AdminUser } from '../../api/admin'
...
export default function AdminProfile() {
  ...
  const initials = admin
    ? admin.name.split(' ').map(p => p[0]).filter(Boolean).slice(0, 2).join('').toUpperCase()
    : '—'
      ...
      <div className="sticky top-0 z-30 -mx-4 md:-mx-6 lg:-mx-8 pl-16 pr-4 md:pl-16 md:pr-6 lg:px-8 bg-foreground md:bg-white/90 md:backdrop-blur-xl border-b border-border md:border-gray-200 shadow-sm mb-6">
        <div className="flex items-center h-20">
          <div>
            ...
            <p className="text-[13px] text-gray-300 md:text-gray-400 mt-1">{admin ? admin.name : 'Votre profil d\'administrateur'}</p>
          </div>
        ...
        <div className="bg-white border border-gray-100 shadow-sm rounded-2xl overflow-hidden">
          <div className="bg-primary px-6 py-8 flex items-center gap-5">
            ...
            <div className="min-w-0">
              <h2 className="text-lg font-semibold text-white truncate">{admin?.name || '—'}</h2>
              ...
              <span className="md:hidden mt-2 inline-flex items-center gap-1.5 px-3 py-1.5 bg-white/15 ring-1 ring-white/30 text-white text-[10px] font-bold uppercase tracking-wider rounded-full">
                <HiShieldCheck size={13} /> {roleLabel(admin?.role)}
              </span>
            </div>
            ...
            <div className="divide-y divide-gray-100/80">
              ...
              <div className="flex items-center gap-4 px-6 py-4">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <span className="text-sm font-medium text-text">{fmtDate(admin.createdAt)}</span>
                </div>
              </div>
              <div className="px-6 py-4">
                <button onClick={() => navigate('/admin/admins/settings')} className="h-10 px-4 bg-foreground hover:scale-105 text-white text-xs font-bold uppercase tracking-wider rounded-lg transition-transform cursor-pointer flex items-center gap-1.5">
