...
import { fetchMe, updateAdminProfile, changeAdminPassword, type AdminUser } from '../../api/admin'
...
export default function AdminSettings() {
  ...
  async function handleSaveProfile(e: React.FormEvent) {
    ...
    try {
      const data = await updateAdminProfile({ name: name.trim(), email: email.trim() })
      setAdmin(data.admin)
  ...
  const inputClass = 'w-full h-11 px-4 bg-gray-50 border border-gray-200 text-sm text-text rounded-lg placeholder-gray-400 focus:outline-none focus:border-primary transition-all'
      ...
      <div className="sticky top-0 z-30 -mx-4 md:-mx-6 lg:-mx-8 pl-16 pr-4 md:pl-16 md:pr-6 lg:px-8 bg-foreground md:bg-white/90 md:backdrop-blur-xl border-b border-border md:border-gray-200 shadow-sm mb-6">
        <div className="flex items-center h-20">
          <div>
            ...
            <p className="text-[13px] text-gray-300 md:text-gray-400 mt-1">Paramètres de votre profil d'administrateur</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 border-b border-gray-100 mb-6">
        {tabs.map(t => (
          <button
            ...
            className={`inline-flex items-center gap-1.5 px-4 py-2.5 text-xs font-bold uppercase tracking-wider transition-all cursor-pointer border-b-2 -mb-px ${
              tab === t.key
        ...
        {tab === 'profile' ? (
          <div className="bg-white border border-gray-100 shadow-sm rounded-2xl">
            ...
            {loading ? (
              ...
              <form onSubmit={handleSaveProfile} className="p-5 space-y-5">
                {profileError && (
                  <div className="bg-red-50 border border-red-200 text-red-600 text-sm px-4 py-3 rounded-lg">{profileError}</div>
                )}
                {profileSuccess && (
                  <div className="bg-green-50 border border-green-200 text-green-700 text-sm px-4 py-3 rounded-lg flex items-center gap-2"><HiCheck size={14} /> {profileSuccess}</div>
                )}
                ...
                <div className="space-y-1.5">
                  ...
                  <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="admin@exemple.com" className={inputClass} />
                </div>
                <div className="space-y-1.5">
                  <label className="flex items-center gap-1.5 text-xs font-semibold text-text uppercase tracking-wider">
                  ...
                  <div className="h-11 px-4 bg-gray-50 border border-gray-200 rounded-lg flex items-center gap-2 text-sm text-text">
                    ...
                    <span className="font-medium">{roleLabel(admin?.role)}</span>
                    <span className="text-[11px] md:text-xs text-gray-400 ml-auto">Non modifiable</span>
                  </div>
                </div>
                <button type="submit" disabled={profileSaving} className="h-10 px-5 bg-foreground hover:scale-105 disabled:hover:scale-100 disabled:opacity-60 text-white text-xs font-bold uppercase tracking-wider rounded-lg transition-transform cursor-pointer disabled:cursor-not-allowed">
          ...
          <div className="bg-white border border-gray-100 shadow-sm rounded-2xl">
            ...
            <form onSubmit={handleChangePassword} className="p-5 space-y-4">
              {passwordError && (
                <div className="bg-red-50 border border-red-200 text-red-600 text-sm px-4 py-3 rounded-lg">{passwordError}</div>
              )}
              {passwordSuccess && (
                <div className="bg-green-50 border border-green-200 text-green-700 text-sm px-4 py-3 rounded-lg flex items-center gap-2"><HiCheck size={14} /> {passwordSuccess}</div>
              )}
              <div className="space-y-1.5">
              ...
              <button type="submit" disabled={passwordSaving} className="h-10 px-5 bg-foreground hover:scale-105 disabled:hover:scale-100 disabled:opacity-60 text-white text-xs font-bold uppercase tracking-wider rounded-lg transition-transform cursor-pointer disabled:cursor-not-allowed">
