...
const topLinks: LinkItem[] = [
  { to: '/employe/performance', label: 'Mon travail', icon: HiChartBar },
  { to: '/employe/orders', label: 'Commandes', icon: HiShoppingBag },
]

const accountLinks: LinkItem[] = [
  { to: '/employe/profile', label: 'Profil', icon: HiUserCircle },
  { to: '/employe/settings', label: 'Paramètres', icon: HiCog6Tooth },
]

function ExpandableSection({ title, icon: Icon, links, onNavigate }: { title: string; icon: typeof HiShoppingBag; links: LinkItem[]; onNavigate?: () => void }) {
  const [open, setOpen] = useState(true)
  const location = useLocation()
  const isActive = links.some(l => location.pathname === l.to || location.pathname.startsWith(l.to + '/'))
        ...
        <div className="pl-3 space-y-0.5 pt-0.5">
          {links.map((link) => (
            <NavLink
              ...
              onClick={onNavigate}
              className={({ isActive }) =>
                `flex items-center gap-3 px-3 py-2.5 lg:py-2 rounded-lg text-sm font-medium transition-all ${
                  isActive
                    ? 'bg-accent text-white'
  ...
  const sidebar = (
    <div className="flex flex-col h-full">
      ...
      <nav className="flex-1 py-4 px-3 space-y-1 overflow-y-auto sidebar-scroll">
        ...
        <ExpandableSection title="Mon compte" icon={HiUsers} links={accountLinks} onNavigate={() => setSidebarOpen(false)} />
      </nav>

      <div className="px-3 pb-4 border-t border-white/10 pt-4">
        <button
          onClick={handleLogout}
          className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-gray-400 hover:text-white hover:bg-white/5 transition-all w-full cursor-pointer"
        >
